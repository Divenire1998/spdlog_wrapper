 ...
 public:
  ...
  struct LogStream : public std::ostringstream {
   public:
    ...
    void Flush() { Logger::Get().Log(loc_, lvl_, (prefix_ + str()).c_str()); }
 ...
 public:
  ...
  bool Init(const std::string& log_file_name, spdlog::level::level_enum lvl = spdlog::level::trace) {
    ...
    try {
      ...
      log_level_ = lvl;
      std::cout << "using log level " << log_level_ << std::endl;
      // 立即输出等级
  ...
  spdlog::level::level_enum GetLevel() { return log_level_; }
  ...
  static void SetFlushOn(spdlog::level::level_enum lvl) { spdlog::flush_on(lvl); }
